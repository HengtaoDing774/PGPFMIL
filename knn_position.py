...
def pt2graph(wsi_h5, wsi_pt, radius=49):
    coords= np.array(wsi_h5['coords'])
    features = np.array(wsi_pt)
...
def make_parse():
    ...
    parser.add_argument('--h5-path', type=str, default= 'F:\WSI_Code\\result_testPatch\patches')
    parser.add_argument('--pt-path', type=str, default='F:\WSI_Code\HVTSurv-main\Camelyon\pt_files')
    parser.add_argument('--save-path', type=str, default= 'F:\WSI_Code\Camelyon_pt_km')
...
if __name__ == '__main__':
    ...
    createDir_h5toPyG(h5_path, pt_path, save_path, radius)
